 ...
 */
public record ModifierParticlePosition(int floor,
                                       int groupID,
                                       double duration,
                                       @Nonnull Point3D destination,
                                       FunctionEase ease,
                                       double angleOffset) implements M3CModifier<ModifierParticlePosition, Point3D> {

    @Override
    public Class<ModifierParticlePosition> type() {
        return ModifierParticlePosition.class;
    }

    @Override
    public ModifierBuilder<ModifierParticlePosition,Point3D> edit() {
        return new Builder(floor, groupID, duration, destination, ease, angleOffset);
    }

    public static final class Builder implements ModifierBuilder<ModifierParticlePosition,Point3D> {
        private int floor;
        private int groupID;
        ...
        @Override
        public ModifierParticlePosition build() {
            return new ModifierParticlePosition(floor, groupID, duration, destination, ease, angleOffset);
        }
    }
}
