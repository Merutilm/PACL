 ...
 */
public record ModifierParticleCenterRotatePosition(int floor,
                                                   int groupID,
                                                   double duration,
                                                   @Nonnull Point3D destination,
                                                   FunctionEase ease,
                                                   double angleOffset) implements M3CModifier<ModifierParticleCenterRotatePosition, Point3D> {

    @Override
    public Class<ModifierParticleCenterRotatePosition> type() {
        return ModifierParticleCenterRotatePosition.class;
    }

    @Override
    public ModifierBuilder<ModifierParticleCenterRotatePosition, Point3D> edit() {
        return new Builder(floor, groupID, duration, destination, ease, angleOffset);
    }

    public static final class Builder implements ModifierBuilder<ModifierParticleCenterRotatePosition, Point3D> {
        private int floor;
        private int groupID;
        ...
        @Override
        public ModifierParticleCenterRotatePosition build() {
            return new ModifierParticleCenterRotatePosition(floor, groupID, duration, destination, ease, angleOffset);
        }
    }
}
