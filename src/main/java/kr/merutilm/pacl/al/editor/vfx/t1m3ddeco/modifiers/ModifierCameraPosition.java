 ...
 */
@SingleGroupModifier
public record ModifierCameraPosition(int floor,
                                     double duration,
                                     @Nonnull Point3D destination,
                                     FunctionEase ease,
                                     double angleOffset) implements M3CModifier<ModifierCameraPosition, Point3D> {

    @Override
    public Class<ModifierCameraPosition> type() {
        return ModifierCameraPosition.class;
    }
    ...
    public ModifierBuilder<ModifierCameraPosition, Point3D> edit() {
        return new Builder(floor, duration, destination, ease, angleOffset);
    }

    public static final class Builder implements ModifierBuilder<ModifierCameraPosition, Point3D> {
        private int floor;
        ...
        @Override
        public ModifierCameraPosition build() {
            return new ModifierCameraPosition(floor, duration, destination, ease, angleOffset);
        }
    }
}
